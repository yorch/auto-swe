...
| `docs/claude-tag.md`             | Current    | Claude-Tag-style Slack channel teammate: one shared `@Claude` per channel. Adds `SlackWorkspace`/`SlackChannel`/`ChannelMonthlyUsage` models + a `CHANNEL` config-scope tier (`WORKFLOW_TEMPLATE → CHANNEL → TEAM → ORGANIZATION → GLOBAL`). **All phases shipped:** foundation · P0 (`@mention` → `ChannelAssistantWorkflow` → in-thread reply via `/api/v1/auth/slack/events`) · P1 (per-channel agent/tool scoping, admin CRUD at `/admin/slack-channels`, soft budget caps) · P2 (channel-scoped team memory + auto-inject + admin view/delete) · P3 (ambient mode: per-channel Temporal Schedule → proactive `ChannelAmbientWorkflow` digest) · P4 (live `chat.update` progress + advisory injection scan of channel input). Future refinements (long-lived signal workflow, full thread-history context) noted in the doc |
