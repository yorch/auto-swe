...
| `docs/nl-workflow-authoring.md`  | Current    | Natural-language workflow authoring: describe an automation → the seeded `workflowAuthor` agent generates a `WorkflowSpec` via a catalog-constrained generate→validate→repair loop in the worker → saved as a DRAFT for canvas review/activation. Entry points: web canvas ("Generate with AI"), gateway `POST /workflow-templates/generate` + CLI `workflows generate`, and the Slack channel assistant's `generateWorkflow` tool |
