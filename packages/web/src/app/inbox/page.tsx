...
import { useInbox, useRespondToHumanStep } from '@/hooks/useWorkflows';
...
export default function InboxPage() {
  ...
  function handleRespond(id: string, action: string, value?: unknown) {
    respond.mutate({ action, id, value });
    setExpanded(null);
  }
                ...
                {step.kind === 'APPROVAL' && (
                  <div className="flex gap-2">
                    <button
                      ...
                      onClick={() => handleRespond(step.id, 'approve')}
                      type="button"
                    >
                      Approve
                    </button>
                    <button
                      ...
                      disabled={respond.isPending}
                      onClick={() => handleRespond(step.id, 'reject')}
                      type="button"
                    >
                      Reject
                    </button>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const value = Object.fromEntries(
                          (fields ?? []).map((f) => [
                            f.key,
                            inputValues[`${step.id}.${f.key}`] ?? '',
                          ])
                        );
                        handleRespond(step.id, 'submit', value);
                      }}
                      type="button"
                    >
                      Submit
                ...
                {step.kind === 'REVIEW' && (
                  <div className="space-y-2">
                    ...
                    <button
                      className="px-4 py-1.5 bg-blue-600 text-white text-sm rounded hover:bg-blue-700 disabled:opacity-50"
                      disabled={respond.isPending}
                      onClick={() => handleRespond(step.id, 'submit', reviewText[step.id] ?? '')}
                      type="button"
                    >
                      Submit
                    </button>
