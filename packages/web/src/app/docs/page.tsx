  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {docs.map((doc) => (
          <Link
            className="block group focus:outline-none focus:ring-2 focus:ring-[var(--primary)] rounded-lg"
            href={`/docs/${doc.slug}`}
            key={doc.slug}
          >
            <Card className="h-full transition-shadow group-hover:shadow-md">
              ...
              {doc.description ? (
                <p className="text-sm text-[var(--muted-foreground)] mt-2">{doc.description}</p>
              ) : null}
