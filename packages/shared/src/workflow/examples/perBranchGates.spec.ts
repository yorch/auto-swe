 ...
 * goes `subImpl → subLint → subTypecheck → subTests → subDone` instead of
 * just `subImpl → subDone`. A failing gate inside a branch terminates that
 * branch FAILED (because the gate's `onFail` is `'block'`), which the parent
 * picks up via `onBranchFail: 'block'`. The blocking semantics mean the run
 * dies fast at the first bad branch instead of merging green ones with red
 * sibling branches sitting unreviewed.
 *
...
export const PER_BRANCH_GATES_EXAMPLE_SPEC: WorkflowSpec = {
  description:
    'Example workflow: decompose → fan-out implementer + per-branch lint/typecheck/tests → merge → review.',
  entry: 'plan',
  name: 'engineering-with-per-branch-gates',
  nodes: {
    done: {
      result: { targetBranch: { from: 'context.featureBranch' } },
      status: 'SUCCESS',
      type: 'terminate',
    },
    fanOutSubtasks: {
