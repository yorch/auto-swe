 ...
 *
 * These are the canonical defaults used both by the worker (as the hardcoded
 * fallback when no DB override is set) and by the web admin UI (to pre-fill
 * the "Load default" button in the Roles modal).
 *
 * Only the six role-level defaults belong here — fix-loop prompts
 * (CI_FIX, GATE_FIX, REVIEW_FIX) and internal reviewer specializations
 * (SECURITY_AUDITOR, PERFORMANCE_REVIEWER) remain in worker/src/agents/prompts.ts.
 */
