...
```bash
# Initialize Yarn 4 via corepack (ships with Node.js 24+)
corepack enable
...
export async function createOrUpdatePullRequest(
  ...
  // Create new PR (title and body are configurable via env vars)
...
//   {{ticketId}}    — e.g., "JIRA-1234"
//   {{description}} — work request description
//   {{branch}}      — e.g., "auto/JIRA-1234"
//   {{testStatus}}  — e.g., "All tests passing (5/5)"
//   {{filesChanged}} — number of files changed
//   {{fileList}}    — markdown list of changed files
//   {{notes}}       — agent implementation notes
//
// Defaults (used when env vars are not set):
//   PR_TITLE_TEMPLATE="[auto-swe] {{ticketId}}"
//   PR_BODY_TEMPLATE (see defaultPRBodyTemplate below)
