...
| Agent          | Implementer Agent (Mastra 1.32 + `claude-opus-4-8`) with bash and GitHub MCP tools                                     |
...
│  │    │    ├─ Clone target repo                               │  │
│  │    │    ├─ Mastra Implementer Agent (claude-opus-4-8)      │  │
...
The only LLM agent in the MVP. Powered by Mastra 1.32 with `claude-opus-4-8`.
...
| `ANTHROPIC_API_KEY`     | worker          | For `claude-opus-4-8` (Implementer)                                                |
