...
| **P2 — declarative `agent` node + MCP** | Partial | WS1 (`agent` node + `runAgentNode`) **done**; WS2 (`'mcp'` tool key) **done**; WS3 **done** (first-class `mcp` Connection; MCP binding across all three implementer activities + the generic `runAgentNode` path via `buildImplementerForActivity`; non-git read/submit paths filtered + guarded by `isGitRepoConnection`; admin write-path — `/api/v1/admin/mcp-connections` + `/admin/mcp-connections` UI + `mcpConnectionId` agent field with `validateMcpConnectionRef` tenancy check); WS4 **done** (`mcp` workflow node — `McpNodeSchema` + interpreter dispatch → `mcpCallTool` activity); remaining: WS5 (canvas palette/inspector for the new node types). |
